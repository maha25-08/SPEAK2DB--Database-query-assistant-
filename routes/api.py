...
# Removed conflicting route to avoid BuildError
...
def books_list():
    ...
    logger.info("api/books GET accessed by role: %s", session.get("role"))
    try:
        conn = get_db_connection(MAIN_DB)
        rows = conn.execute(
...
def books_add():
    ...
    # Ensure category is not empty (database constraint)
    if not category:
        category = "General"
    try:
