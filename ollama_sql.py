...
_RULE_DICT = [
    ...
    ("unpaid fines",
     "SELECT f.id AS fine_id, f.student_id, f.fine_amount, f.fine_type, f.status, f.issue_date FROM Fines f WHERE f.status = 'Unpaid'"),
    ...
    ("show students who have unpaid fines",
     ...
     "JOIN Fines f ON s.id = f.student_id WHERE f.status = 'Unpaid'"),
    ...
    ("show total fine amount per student",
     "SELECT s.id,s.name,s.roll_number,f.fine_amount,f.status,f.issue_date "
     "FROM Students s LEFT JOIN Fines f ON s.id = f.student_id "
     "WHERE f.status = 'Unpaid' ORDER BY f.issue_date DESC"),
    ("total fine amount per student",
     "SELECT s.id,s.name,s.roll_number,f.fine_amount,f.status,f.issue_date "
     "FROM Students s LEFT JOIN Fines f ON s.id = f.student_id "
     "WHERE f.status = 'Unpaid' ORDER BY f.issue_date DESC"),
    ("fines per student",
     "SELECT s.id,s.name,s.roll_number,SUM(f.fine_amount) as total_fine_amount,f.status,f.issue_date "
     "FROM Students s LEFT JOIN Fines f ON s.id = f.student_id "
     "WHERE f.status = 'Unpaid' GROUP BY s.id,s.name,s.roll_number,f.status,f.issue_date "
    ...
    ("list departments",
     "SELECT id,name FROM Departments"),
    # ── Librarian UPDATE Operations ───────────────────────────────────────────
    ("mark book as returned",
     "UPDATE Issued SET return_date = date('now') WHERE return_date IS NULL"),
    ("book returned",
     "UPDATE Issued SET return_date = date('now') WHERE return_date IS NULL"),
    ("update return status",
     "UPDATE Issued SET return_date = date('now') WHERE return_date IS NULL"),
    ("mark fine as paid",
     "UPDATE Fines SET status = 'Paid', payment_date = date('now'), payment_method = 'Cash' WHERE status = 'Unpaid'"),
    ("fine paid",
     "UPDATE Fines SET status = 'Paid', payment_date = date('now'), payment_method = 'Cash' WHERE status = 'Unpaid'"),
    ("update fine status",
     "UPDATE Fines SET status = 'Paid', payment_date = date('now'), payment_method = 'Cash' WHERE status = 'Unpaid'"),
    ("record fine payment",
     "UPDATE Fines SET status = 'Paid', payment_date = date('now'), payment_method = 'Cash' WHERE status = 'Unpaid'"),
    ("mark fine as paid by cash",
     "UPDATE Fines SET status = 'Paid', payment_date = date('now'), payment_method = 'Cash' WHERE status = 'Unpaid'"),
    ("mark fine as paid by card",
     "UPDATE Fines SET status = 'Paid', payment_date = date('now'), payment_method = 'Card' WHERE status = 'Unpaid'"),
    ("mark fine as paid by online",
     "UPDATE Fines SET status = 'Paid', payment_date = date('now'), payment_method = 'Online' WHERE status = 'Unpaid'"),
    ("mark all fines as paid",
     "UPDATE Fines SET status = 'Paid', payment_date = date('now'), payment_method = 'Cash' WHERE status = 'Unpaid'"),
    ("all fines paid",
     "UPDATE Fines SET status = 'Paid', payment_date = date('now'), payment_method = 'Cash' WHERE status = 'Unpaid'"),
    ("mark all books as returned",
     "UPDATE Issued SET return_date = date('now') WHERE return_date IS NULL"),
    ("all books returned",
     "UPDATE Issued SET return_date = date('now') WHERE return_date IS NULL"),
]
...
def _match_rule_dict(query: str) -> str:
    ...
    for phrase, sql in _RULE_DICT:
        if phrase in q_lower:
            ...
            # Check if it's an UPDATE query and validate safety
            if sql.strip().upper().startswith("UPDATE"):
                if _is_safe_update_sql(sql):
                    return sql
                else:
                    print(f"[UNSAFE UPDATE] Rule-based UPDATE query rejected: {sql}")
                    continue
            else:
                return sql
    return ""
...
def _is_safe_update_sql(sql: str) -> bool:
    """Return True only if *sql* is a safe UPDATE query for librarian operations."""
    stripped = sql.strip()
    ...
    allowed_patterns = [
        "UPDATE Issued SET return_date",
        "UPDATE Fines SET status",
        "UPDATE Fines SET payment_date",
        "UPDATE Fines SET payment_method"
    ]
...
def generate_update_sql(user_query):
    """
    Generate UPDATE SQL for librarian operations
    """
    query_lower = user_query.lower()
    
    # ── BOOK RETURN UPDATES ─────────────────────────────────────────────
    if "mark book as returned" in query_lower or "book returned" in query_lower:
        if "student" in query_lower and "roll number" in query_lower:
            return "UPDATE Issued SET return_date = date('now') WHERE student_id IN (SELECT id FROM Students WHERE roll_number = ?) AND return_date IS NULL"
        elif "book" in query_lower and "title" in query_lower:
            return "UPDATE Issued SET return_date = date('now') WHERE book_id IN (SELECT id FROM Books WHERE title LIKE ?) AND return_date IS NULL"
        else:
            return "UPDATE Issued SET return_date = date('now') WHERE return_date IS NULL"
    
    elif "update return status" in query_lower or "return status" in query_lower:
        return "UPDATE Issued SET return_date = date('now') WHERE return_date IS NULL"
    
    # ── FINE PAYMENT UPDATES ─────────────────────────────────────────────
    elif "mark fine as paid" in query_lower or "fine paid" in query_lower:
        if "student" in query_lower and "roll number" in query_lower:
            return "UPDATE Fines SET status = 'Paid', payment_date = date('now'), payment_method = 'Cash' WHERE student_id IN (SELECT id FROM Students WHERE roll_number = ?) AND status = 'Unpaid'"
        elif "fine id" in query_lower:
            return "UPDATE Fines SET status = 'Paid', payment_date = date('now'), payment_method = 'Cash' WHERE id = ? AND status = 'Unpaid'"
        else:
            return "UPDATE Fines SET status = 'Paid', payment_date = date('now'), payment_method = 'Cash' WHERE status = 'Unpaid'"
    
    elif "update fine status" in query_lower or "fine status" in query_lower:
        return "UPDATE Fines SET status = 'Paid', payment_date = date('now'), payment_method = 'Cash' WHERE status = 'Unpaid'"
    
    elif "record fine payment" in query_lower or "fine payment" in query_lower:
        return "UPDATE Fines SET status = 'Paid', payment_date = date('now'), payment_method = 'Cash' WHERE status = 'Unpaid'"
    
    # ── SPECIFIC PAYMENT METHODS ─────────────────────────────────────────
    elif "mark fine as paid by cash" in query_lower or "paid by cash" in query_lower:
        return "UPDATE Fines SET status = 'Paid', payment_date = date('now'), payment_method = 'Cash' WHERE status = 'Unpaid'"
    
    elif "mark fine as paid by card" in query_lower or "paid by card" in query_lower:
        return "UPDATE Fines SET status = 'Paid', payment_date = date('now'), payment_method = 'Card' WHERE status = 'Unpaid'"
    
    elif "mark fine as paid by online" in query_lower or "paid by online" in query_lower:
        return "UPDATE Fines SET status = 'Paid', payment_date = date('now'), payment_method = 'Online' WHERE status = 'Unpaid'"
    
    # ── BATCH OPERATIONS ─────────────────────────────────────────────────
    elif "mark all fines as paid" in query_lower or "all fines paid" in query_lower:
        return "UPDATE Fines SET status = 'Paid', payment_date = date('now'), payment_method = 'Cash' WHERE status = 'Unpaid'"
    
    elif "mark all books as returned" in query_lower or "all books returned" in query_lower:
        return "UPDATE Issued SET return_date = date('now') WHERE return_date IS NULL"
...
def generate_sql(user_query):
    ...
    # ── Layer 4: UPDATE operations for librarians ───────────────────────────
    sql = generate_update_sql(clean_query)
    if sql:
        if _is_safe_update_sql(sql):
            return sql
        else:
            print(f"[UNSAFE UPDATE] Generated UPDATE query rejected: {sql}")
            return ""
...
def generate_complex_sql(user_query):
    ...
    # EXTENSIVE STUDENT PATTERNS
        ...
        return "SELECT s.name, f.fine_amount, f.status FROM Students s LEFT JOIN Fines f ON s.id = f.student_id WHERE s.id = [CURRENT_STUDENT_ID] AND f.status = 'Unpaid'"
    ...
    # COUNT queries
        ...
        if "books" in query_lower and "category" in query_lower:
            return "SELECT category, COUNT(*) as book_count FROM Books GROUP BY category ORDER BY book_count DESC"
        elif "sum" in query_lower and "books" in query_lower:
            return "SELECT category, SUM(price) as total_price FROM Books GROUP BY category ORDER BY total_price DESC"
